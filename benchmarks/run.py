"""Run a benchmark through the (WIP) inference adapter.
...
NOTE: numbers are PLACEHOLDERS until the inference port is finished (docs/inference.md). Pass
--strict to hard-fail instead of producing placeholders. BABILong also needs a *fine-tuned*
checkpoint to be meaningful (see benchmarks/babilong.py).
"""
import argparse
import os
import time
...
def main():
    ...
    ap.add_argument("--strict", action="store_true",
                    help="hard-fail if the engine can't run this checkpoint (no placeholders)")
    args = ap.parse_args()
    ...
    model = EvalModel(args.model, max_tokens=args.max_tokens, strict=args.strict)
