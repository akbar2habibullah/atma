"""Multi-GPU ablation worker: claim configs from a shared directory and train them.

    # one worker per GPU (run several, one per device); they coordinate via atomic file claims
    python -m ablation.run_worker --config_dir ablation/configs --log_dir ablation/logs --gpu 0
    python -m ablation.run_worker --config_dir ablation/configs --log_dir ablation/logs --gpu 1
    ...

    python -m ablation.run_worker --reset      # un-claim crashed/failed configs back to pending
...
def main():
    ...
    ap.add_argument("--config_dir", default="ablation/configs")
    ...
    while True:
        ...
        rc = subprocess.run([sys.executable, "-m", "ablation.train",
                             "--config", claimed, "--log", log_path], env=env).returncode
