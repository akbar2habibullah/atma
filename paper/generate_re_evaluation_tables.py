...
ARXIV_OUT_FILE = ROOT / "paper" / "arxiv" / "gamma_re_evaluation_tables.tex"
...
def generate_haystack_exact_retrieval_breakdown_table():
    jobs = json.loads(RE_EVAL_FILE.read_text(encoding="utf-8"))["results"]
    capped_jobs = {(job["family"], job["model"], job["suite"]): job["result"] for job in jobs}
    ...
    base_grouped = defaultdict(list)
    for r in base_rows:
        if r.get("benchmark") != "retrieval" or r.get("metric") != "exact_match":
            continue
        if "/smoke_" in r.get("source_log", "") or r.get("model") not in MODELS:
            continue
        suite = r.get("suite")
    ...
    for model in MODELS:
        for idx, suite in enumerate(["synthetic", "real"]):
            ...
            prefix = rf"\multirow{{2}}{{*}}{{{LABELS[model]}}}" if idx == 0 else ""
            cells = []
            payload = capped_jobs[("retrieval", model, suite)]["exact_results"]
            for length in LENGTHS:
                b_vals = base_grouped[(model, suite, length)]
                b_mean = sum(b_vals) / len(b_vals) if b_vals else 0.0
                c_vals = []
...
def main():
    ...
    # 5. Haystack retrieval breakdown (token and exact)
    sections.append(generate_haystack_retrieval_breakdown_table())
    sections.append(generate_haystack_exact_retrieval_breakdown_table())
    ...
    ARXIV_OUT_FILE.write_text(content, encoding="utf-8")
    print(f"wrote {ARXIV_OUT_FILE}")
