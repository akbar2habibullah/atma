...
\subsubsection{Numerical oracle.}
...
\subsubsection{Streaming sufficient statistics.}
...
\subsubsection{Raven optimizer control.}
...
\begin{landscape}
\subsubsection{Single-sequence scaling.}
...
\subsubsection{Production-framework stress comparison.}
...
\section{Environment-Sensitivity Diagnostic Journey}
\label{appendix:environment}

The sensitivity finding emerged sequentially. Stage I used L4 GPUs, followed by the first Stage II 10B NoPE control on L4. The anomaly became visible only after the remaining Stage II training moved to L40S: short-context convergence remained nearly unchanged while the 256K extrapolation results diverged. Table~\ref{table:environment_diagnostic_journey} records the subsequent elimination sequence.
