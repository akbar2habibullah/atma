...
\paragraph{Streaming sufficient statistics.}
...
Figure~\ref{fig:retrieval_depth_sweeps} exposes depth-wise variation, while Table~\ref{table:retrieval_breakdown} aggregates the same retrieval evaluation by haystack and context length.
...
\begin{landscape}
\paragraph{Single-sequence scaling.}
...
\subsection{Environment-Sensitivity Diagnostic Journey}

The sensitivity finding emerged sequentially. Stage I used L4 GPUs, followed by the first Stage II 10B NoPE control on L4. The anomaly became visible only after the remaining Stage II training moved to L40S: short-context convergence remained nearly unchanged while the 256K endpoint diverged. Table~\ref{table:environment_diagnostic_journey} records the subsequent elimination sequence.
...
\midrule
L4 baseline & Stage I and the first Stage II 10B NoPE control used L4, microbatch 4; 256K NLL was 4.07. & Established the original extrapolation behavior, but not a replicated device-class estimate. \\
...
All five headline Stage II models were trained in the same L40S software and hardware environment, so their primary comparison is internally matched. The cross-device observation nevertheless motivates a broader audit principle: long-context extrapolation claims should report device class, kernels, seeds, and extended-context diagnostics rather than infer robustness from short-context loss alone. The present evidence does not identify CUDA reduction order, floating-point accumulation, or any other single mechanism as causal, because the L4/L40S intervention was not randomized or replicated across multiple seeds and kernel configurations.
