...
\toprule
\textbf{Model} & \textbf{Haystack} & \multicolumn{3}{c}{\textbf{Token accuracy (\%)}} & \multicolumn{3}{c}{\textbf{Exact five-token accuracy (\%)}} \\
\cmidrule(lr){3-5}\cmidrule(lr){6-8}
& & \textbf{2K} & \textbf{64K} & \textbf{256K} & \textbf{2K} & \textbf{64K} & \textbf{256K} \\
\midrule
...
\begin{itemize}
  ...
  \item \textbf{Retrieval Curves (Tables~\ref{table:gamma_retrieval_token_full}, \ref{table:gamma_retrieval_exact_full}, \ref{table:gamma_haystack_retrieval_full}):} Polar gains $+12.70$ points target-token accuracy and $+7.33$ points exact five-token accuracy at 256K, including $+9.1\%$ synthetic token accuracy and $+16.3\%$ FinePDFs real-text accuracy. For NoPE, synthetic token retrieval recovers ($0.6\% \rightarrow 1.9\%$), but exact retrieval remains $0.0\%$, cleanly decoupling state stability from exact associative recall.
