...
\caption{Untouched zero-shot accuracy on eight 2K controls (22,939 examples per model). The rightmost group gives unweighted task means. Raven variants are separately optimized references. Table~\ref{table:gamma_downstream_full} reports the inference-cap comparison.}
...
\toprule
\textbf{Group} & \textbf{Model} & \textbf{Base mean} & \textbf{128K prefill} & \textbf{Decode} & \textbf{Engine alloc.} & \textbf{Train MFU} \\
& & (\%) & (s) & (ms/token) & (GiB) & (\%, 2K) \\
\midrule
...
\bottomrule
\end{tabular}}
\caption{Short-context and systems trade-offs. Serving uses one sequence and one timing sample on one L40S. Engine allocation includes the attention backend's preallocated KV pool (90\% device-memory budget), so it is not the memory required by a single 128K sequence. Appendix~\ref{appendix:systems} distinguishes pool allocation from occupied KV state.}
\label{tab:systems}
\end{table}
...
The evidence covers one model scale and 2K pretraining, with two fresh NoPE/Polar pairs and no seed-matched RoPE replication. The factorial varies recipe settings rather than training seeds. Shared retrieval haystacks and 18 fixed BPB targets limit independent natural-text coverage; teacher-forced retrieval, adapted BABILong, and single-sample serving answer different questions. Exact FinePDFs retrieval reaches zero at extreme lengths in every tested model. Raven changes model family and optimizer. We have no matched 9.816B-token comparison with TDA or a softmax control sharing Polar's learned temperature, and the component pilot does not establish an incremental benefit for the full decomposition. Its favorable fixed-null result requires replication and calibration measurements.

The recurrent bound assumes a uniform retention margin that the learned sigmoid does not enforce. The cap is a checkpoint intervention: it does not identify the outlier's training origin or validate a bounded train-time formulation. Broad re-evaluation compares capped runs with pinned archived baselines; the smaller sweeps and fresh replications provide controlled pairing. We therefore distinguish replicated NoPE likelihood repair from checkpoint-specific Polar recovery.

ATMA establishes two empirical findings within this scope. Recurrent memory improves Polar retrieval across every matched factorial cell, and Polar retains a target-token advantage over NoPE in both fresh pairs, with weaker exact retrieval than in the primary checkpoint. Separately, a single retention-gate intervention repairs NoPE's long-context likelihood collapse in the primary and both fresh runs despite similar short-context convergence. The component pilot and Raven references delimit the attribution and quality--systems trade-offs. Released configurations, logs, checkpoint manifests, benchmark matrices, clamp specifications, and proof skeletons make these findings auditable.
