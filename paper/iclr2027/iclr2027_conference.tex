...
When evaluated on exact associative recall in natural text (needle-in-a-haystack), unadapted length extrapolation exposes a harsh, architecture-wide reality: exact retrieval degrades to zero across all standard baselines. Recurrent state architectures such as Raven exhibit an almost flat zero exact retrieval rate across sequence lengths (0.0\% on real text from 2K through 256K), while positional baselines like RoPE and NoPE suffer steep cliff collapses by 4K and 8K. Polar Attention is the only configuration that endures significantly into extended context, sustaining exact natural-text recall out to 8K--16K and partial signal to 256K before the universal real-text noise floor takes over.
...
\caption{Full length-wise re-evaluation with both recurrent-family comparisons retained. For matched NoPE, Polar, and RoPE, dashed open curves are untouched checkpoints and solid curves apply the inference-only one-head cap. Dotted filled curves are untouched Raven Native and Atma-Raven-Titans references; they are not clamped or optimizer-matched ablations. \textbf{Left:} teacher-forced target-token retrieval. \textbf{Center:} BABILong exact match after adaptation. \textbf{Right:} mean fixed-target BPB (log scale; lower is better).}
\label{fig:main_results}
\end{figure}

Our contributions are threefold. First, a complete 120-cell, 1B-token factorial sweep measures interactions among attention core, recurrent memory, training-time local windows, distractor alignment, and representation regularization. Second, we promote one recipe to matched 9.816B-token NoPE, RoPE, and Polar runs and evaluate retrieval, BABILong, fixed-target likelihood, short-context tasks, and serving. Raven models provide a strong but separately optimized recurrent-family comparison~\citep{raven2026}; the untouched results locate the original quality--systems frontier. Third, we trace checkpoint-specific extrapolation failure from near-identical 2K loss curves to isolated near-unit retention gates, then test that mechanism with a single-head, inference-only intervention and a full re-evaluation. Figure~\ref{fig:main_results} shows the recovery over every evaluated length rather than only at 256K. The selective recovery identifies excessive retention as a causal mediator in NoPE and Polar, while negative controls and residual failures delimit the claim.
...
Table~\ref{tab:endpoints} reports extrapolation endpoints. At 2K, NoPE and Polar achieve nearly perfect target-token accuracy. At 256K, NoPE and RoPE fall to 0.6\% and 0.0\%, while Polar retains 34.4\% averaged across tasks, suites, and depths. Exact five-token accuracy reveals a fundamental, architecture-wide boundary: collapsing to zero exact accuracy on natural text is not a failure specific to ATMA, but a universal breakdown shared by all baseline recipes. Recurrent Raven models achieve almost flat zero exact retrieval across context lengths (Raven Native scores flat 0.0\% on FinePDFs across all 2K--256K lengths, and $\le 1.0\%$ overall; Atma-Raven-Titans drops from 4.7\% at 2K to 0.0\% at 4K). Positional attention baselines experience precipitous cliff collapses on natural text: RoPE plummets from 77.3\% at 2K to 0.0\% at 4K, and NoPE falls from 96.7\% at 2K to 18.0\% at 4K and 0.0\% at 8K. In sharp contrast, Polar is the \emph{only} configuration that endures the longest: untouched Polar sustains 93.0\% at 2K, 67.3\% at 4K, 62.7\% at 8K, and 12.0\% at 16K on FinePDFs—holding out $4\times$ to $8\times$ longer than the baselines before decaying. On synthetic haystacks, Polar maintains 65.7\% exact retrieval at 64K and 18.0\% at 256K, where all baselines are zeroed out. When retention-capped, Polar's exact real-text accuracy remains above 91\% through 8K (91.3\% at 8K) and preserves non-zero exact match out to 128K (16.3\% at 32K, 1.0\% at 128K), and reaches 32.7\% exact synthetic recall at 256K. Thus, zero exact real-text retrieval at extreme lengths (64K--256K untouched) is the universal ceiling of unadapted length extrapolation, with Polar establishing the single most resilient Pareto frontier. Full token, exact, depth, and haystack matrices are in Appendix~\ref{appendix:retrieval}.
...
\bottomrule
\end{tabular}}
\caption{Zero-shot retrieval token accuracy (\%) by haystack type: untouched $\rightarrow$ capped for matched models; Raven variants are untouched references. Values average teacher-forced token accuracy over needle depths.}
\label{tab:haystack}
\end{table}
...
\end{equation}
emerged as the optimal operating point: it achieved the largest long-distance cross-entropy reduction while strictly satisfying the 2K short-context loss guardrail ($\le 0.05$ nats drift). In this same-process paired sweep, clean NLL at 256K fell from 12.003 to 1.138 for NoPE and from 1.252 to 1.057 for Polar; needle cross-entropy dropped from 12.20 to 6.05 and from 2.39 to 1.71, respectively. RoPE changed only from 6.16 to 6.02 and lost short-range retrieval, providing an empirical negative control.

We then promoted this selected 256-token ceiling to re-benchmark downstream, retrieval, fixed-target BPB, and BABILong across the matched attention variants. Figure~\ref{fig:main_results} plots the complete length curves, Table~\ref{tab:gamma_rebench} gives the endpoints, and Appendix~\ref{appendix:retention_audit} provides full numerical tables. All 15 requested jobs completed without an out-of-memory failure.
...
\bottomrule
\end{tabular}}
\caption{Inference-only retention diagnostic: archived baseline $\rightarrow$ one-head 256-token half-life cap. Base mean averages eight 2K tasks (\%); retrieval is token/exact accuracy (\%); BABI is macro exact match (\%). BPB averages three fixed-target datasets. The cap is a probe, not a proposed trained model.}
\label{tab:gamma_rebench}
\end{table}

The intervention largely removes NoPE's likelihood collapse and raises BABILong from 21/6/0/0/0\% to 54/45/37/35/39\% at 16/32/64/128/256K, but NoPE's exact natural-text retrieval remains zero beyond 8K (and zero overall at 256K). This demonstrates broad extrapolation recovery, while highlighting that state stability alone is insufficient for exact natural-text recall. For Polar, clamping improves 256K token retrieval by 12.70 points, overall exact retrieval by 7.33 points (reaching 32.7\% on synthetic), and BABILong by 14 points; it elevates 8K exact real-text retrieval to 91.3\% (vs 0\% for all baselines) and maintains non-zero exact match through 128K. Its mean BPB degradation falls from $1.26\times$ to $1.03\times$. Ordinary 2K downstream means are unchanged within 0.06 points. RoPE's failure to recover retrieval, and its BABILong decrease at 256K, show that the cap is not a generic benchmark booster. Together these results identify excessive retention as a major checkpoint-specific mediator of NoPE and Polar degradation, while proving that Polar's geometric magnitude-direction channel is essential for sustaining exact associative recall under length extrapolation.
...
\item What training event produces the isolated outlier, and does it replicate under seed-paired device, kernel, and optimizer interventions?
...
\item Does Polar's remaining natural-text retrieval gap at extreme context (256K) improve with longer or mixed-domain training, or reflect a universal unadapted distractor dispersion boundary?
\end{enumerate}
...
This study uses one model scale, one 2K pretraining regime, and no seed replication sufficient for population-level architectural uncertainty. The Stage I factorial demonstrates robustness across nuisance-factor settings, not across random seeds. Teacher-forced retrieval, custom BABILong adaptation, and single-sample serving measurements answer different questions and should not be collapsed into one score. Polar reaches zero exact FinePDFs retrieval at extreme context (64K--256K untouched), but our multi-model evaluation proves this is not an isolated ATMA defect: all tested baselines—from recurrent Raven models that flatline at zero exact retrieval across context lengths, to RoPE and NoPE that suffer immediate 4K/8K cliff collapses—ultimately succumb to zero exact real-text accuracy. Polar is the only configuration that endures the longest across lengths. Raven is not an optimizer-matched ablation. The recurrent bound requires a uniform learned margin that the sigmoid parameterization does not enforce. The one-head cap diagnoses existing checkpoints; it neither identifies the outlier's training origin nor validates a bounded train-time formulation. Its broad re-evaluation uses pinned archived baselines, with controlled pairing supplied by the smaller sweep. The documented pre-integration Polar design tests are not a substitute for network-level component ablations, and the present comparisons do not cover every contemporary length-scaling method.
