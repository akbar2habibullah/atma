...
# Default checkpoint directory (../checkpoints relative to the repo root). A checkpoint
# dir holds weights.pt (+ optional config.json / tokenizer.json).
_DEFAULT_CKPT_DIRS = ["../checkpoints"]
...
def find_checkpoint(explicit=None):
    """Locate a checkpoint. Returns (weights_path, config_path|None, tokenizer_name|None)
    or None. `explicit` may be a weights file or a directory."""
    ...
    if explicit:
        if os.path.isfile(explicit):
            ...
            return explicit, (cfg if os.path.isfile(cfg) else None), _tok_name(d)
        dirs.append(explicit)
    dirs += [os.path.normpath(os.path.join(here, "..", p)) for p in _DEFAULT_CKPT_DIRS]
    for d in dirs:
        ...
        for w in _WEIGHT_NAMES:
            ...
            if os.path.isfile(wp):
                cfg = os.path.join(d, "config.json")
                return wp, (cfg if os.path.isfile(cfg) else None), _tok_name(d)
    return None
...
def main():
    ...
    cfg = info["config"]
    print(f"[checkpoint] {'loaded ' + info['path'] if info['loaded'] else 'NOT FOUND -> random init'}")
    ...
    if args.ids is not None:
        ...
        raise SystemExit("no tokenizer (install tiktoken); pass --ids instead of --prompt")
    else:
