...
if HAS_TRITON:
    ...
    def _polar_fwd_kernel(
        ...
        # per-query length quantities
        n_i = tl.load(NKEYS + offs_m, mask=m_valid, other=1.0).to(tl.float32)
        n_i = tl.maximum(n_i, 1.0)
        ...
        logn = tl.log(n_i)
        temp = 1.0 + spg * logn
        nullv = nb + sps * tl.sqrt(tl.log(n_i + 1.0))
...
def _polar_backward(gc, gm, q, k, v, n_keys, v_null, null_base, null_slope_raw,
    ...
    gc = gc.to(fdt)
    gm = gm.to(fdt)
    n = n_keys.to(dev).to(fdt).clamp(min=1.0)                       # (Tq,)
