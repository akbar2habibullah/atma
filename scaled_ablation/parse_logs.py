"""Parse scaled-ablation .log files into one results.json (one record per run).

    python -m scaled_ablation.parse_logs --log_dir scaled_ablation/logs --out scaled_ablation/results.json
...
def main():
    ap = argparse.ArgumentParser(description="Parse scaled-ablation logs -> results.json")
    ap.add_argument("--log_dir", default="scaled_ablation/logs")
    ap.add_argument("--out", default="scaled_ablation/results.json")
    args = ap.parse_args()
