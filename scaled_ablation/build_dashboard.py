"""Build a self-contained interactive dashboard (single .html, no deps) from scaled-ablation logs.

    python -m scaled_ablation.build_dashboard --log_dir scaled_ablation/logs raven_baseline/scaled_logs --out pages/scaled_dashboard.html
    # or from an existing results.json:
    python -m scaled_ablation.build_dashboard --results scaled_ablation/results.json --out pages/scaled_dashboard.html
...
from scaled_ablation.config_schema import EVAL_LENGTHS, expand_grid, ATTN_TYPES, REG_MODES
...
<title>Atma scaled ablation dashboard</title>
<style>
...
def main():
    ap = argparse.ArgumentParser(description="Build the static scaled-ablation dashboard.")
    ap.add_argument("--log_dir", nargs="+", default=["scaled_ablation/logs"])
    ap.add_argument("--results", default=None, help="use an existing results.json instead of parsing logs")
    ap.add_argument("--out", default="pages/scaled_dashboard.html")
    args = ap.parse_args()
